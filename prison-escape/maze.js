  ...
  checkWin() {
    if (this.player.x == this.end.x && this.player.y == this.end.y) {
      ...
      if (solutionClick) {
        ...
        this.ctx.fillText("I wonder what happen if you do not use a solution button?", 110, this.size*this.squareSize - 20);
...

    {
      ...
      if (this.inbound(cell_top)) {
        if (this.grid[cell_top.x][cell_top.y]) {
          ...
          path[cell_top.x + cell_top.y * this.size].adjacent = visited.getByIndex(0);
      ...
      if (this.inbound(cell_left)) {
        if (this.grid[cell_left.x][cell_left.y]) {
          ...
          path[cell_left.x + cell_left.y * this.size].adjacent = visited.getByIndex(0);
      ...
      if (this.inbound(cell_right)) {
        if (this.grid[cell_right.x][cell_right.y]) {
          ...
          path[cell_right.x + cell_right.y * this.size].adjacent = visited.getByIndex(0);
      ...
      if (this.inbound(cell_bot)) {
        if (this.grid[cell_bot.x][cell_bot.y]) {
          ...
          path[cell_bot.x + cell_bot.y * this.size].adjacent = visited.getByIndex(0);
...
window.onload = () => {
  ...
  prisoner.init();
  prisoner.ctx.drawImage(prisoner.bgImg, 0, 0, 900, 900);
  //play game
  playBtn.onclick = () => {
    solutionClick = false;
    hasPlayed = true;
    prisoner.ctx.clearRect(0, 0, 900, 900);
    if (prisoner.timeOut || prisoner.hasWin) {
      prisoner.div.removeChild(prisoner.canvas);
      prisoner.init();
      prisoner.div.appendChild(prisoner.canvas);
    }
  ...
  aboutBtn.onclick = () => {
    alert(
      "This is a prisoner Maze fangame. It applies the knowledge of finding shortest path, namely the Dijkstra's algorithm to help prisoner find the shortest way to break out of the prison in short time! \n\n Our team: \n Trịnh Bảo Long\n Chu Nguyễn Hoàng Sơn \n Bùi Nam Thuận \n Trịnh Văn Đông \n Tài Anh \n Phạm hiếu"
    );
  };
  //instruction
  instrucBtn.onclick = () => {
    alert(
      "The guards and cameras have noticed you. Quickly find the path and escape from the prison. You only have 30 seconds, so hurry up before it's too late!\n\nHow to play:\nUse Arrow keys to move"
    );
  };
  window.addEventListener("keydown", (e) => {
    if (!(prisoner.hasWin || prisoner.timeOut)) {
      switch (e.key) {
        case "ArrowUp":
          prisoner.moveUp();
          break;
